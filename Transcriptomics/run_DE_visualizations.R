...
upregulated <- list(P5 = T5T0_sig_up$gene, P24 = T24T0_sig_up$gene, U5 = U5U0_sig_up$gene, U24 = U24U0_sig_up$gene)
downregulated <- list(P5 = T5T0_sig_down$gene, P24 = T24T0_sig_down$gene, U5 = U5U0_sig_down$gene, U24 = U24U0_sig_down$gene)
...
sig_genes_groupbatch_v0<- unique(select(rbind(T5T0_sig,U5U0_sig,T24T0_sig,U24U0_sig),gene))

##### heatmap #####

heatmap_sig_groupbatch_v0 <- sig_genes_groupbatch_v0
heatmap_sig_groupbatch_v0$gene <- sort(heatmap_sig_groupbatch_v0$gene)
...
heatmap_sig_groupbatch_v0[heatmap_sig_groupbatch_v0$gene %in% T5T0$gene,]$T5T0 = T5T0[T5T0$gene %in% heatmap_sig_groupbatch_v0$gene,]$log2FoldChange
heatmap_sig_groupbatch_v0[heatmap_sig_groupbatch_v0$gene %in% U5U0$gene,]$U5U0 = U5U0[U5U0$gene %in% heatmap_sig_groupbatch_v0$gene,]$log2FoldChange
heatmap_sig_groupbatch_v0[heatmap_sig_groupbatch_v0$gene %in% T24T0$gene,]$T24T0 = T24T0[T24T0$gene %in% heatmap_sig_groupbatch_v0$gene,]$log2FoldChange
heatmap_sig_groupbatch_v0[heatmap_sig_groupbatch_v0$gene %in% U24U0$gene,]$U24U0 = U24U0[U24U0$gene %in% heatmap_sig_groupbatch_v0$gene,]$log2FoldChange

rownames(heatmap_sig_groupbatch_v0) <- heatmap_sig_groupbatch_v0$gene
heatmap_sig_groupbatch_v0 <- as.matrix(select(heatmap_sig_groupbatch_v0,-gene))
...
enrich_table$asterisk_nudge <- case_when(
  enrich_table$condition == "U5U0"  ~ .35,
  enrich_table$condition == "U24U0" ~ .03,
  enrich_table$condition == "T5T0"  ~ -.2,
  enrich_table$condition == "T24T0" ~ -.35
  ,
  TRUE ~ 0
)
term_order <- enrich_table %>%
  dplyr::distinct(term, genes_in_universe) %>%
  dplyr::arrange(desc(genes_in_universe)) %>%
  dplyr::pull(term)

# apply this order to the term factor
enrich_table$term <- factor(enrich_table$term, levels = term_order)

ggplot(enrich_table, aes(x = term)) +
  scale_y_continuous(limits = c(-25, 25), labels=abs) +
  #geom_bar(aes(y = -log10(adj_pvalue), fill = condition, color = condition), stat = "identity", position = position_dodge(0.75), width=0.5) +
  ...
  geom_text(
    data = subset(enrich_table, adj_pvalue < 0.01),
    aes(
      y = ifelse(perc_set > 0, perc_set + 1, perc_set - 1),
      label = "*"
    ),
    nudge_x = subset(enrich_table, adj_pvalue < 0.01)$asterisk_nudge,
    size = 4,
    color = "black"
  ) +
...
T24T0_metab <- T24T0[which(T24T0$gene %in% metabGenes$PA14_metabolic_model_genes),]
U5U0_metab <- U5U0[which(U5U0$gene %in% metabGenes$PA14_metabolic_model_genes),]
U24U0_metab <- U24U0[which(U24U0$gene %in% metabGenes$PA14_metabolic_model_genes),]
...
# see piping help here: https://stackoverflow.com/questions/13774773/check-whether-value-exist-in-one-data-frame-or-not
heatmap_metab_sig[heatmap_metab_sig$gene %in% T5T0_metab$gene,]$T5T0_metab = T5T0_metab[T5T0_metab$gene %in% heatmap_metab_sig$gene,]$log2FoldChange
heatmap_metab_sig[heatmap_metab_sig$gene %in% U5U0_metab$gene,]$U5U0_metab = U5U0_metab[U5U0_metab$gene %in% heatmap_metab_sig$gene,]$log2FoldChange
heatmap_metab_sig[heatmap_metab_sig$gene %in% T24T0_metab$gene,]$T24T0_metab = T24T0_metab[T24T0_metab$gene %in% heatmap_metab_sig$gene,]$log2FoldChange
heatmap_metab_sig[heatmap_metab_sig$gene %in% U24U0_metab$gene,]$U24U0_metab = U24U0_metab[U24U0_metab$gene %in% heatmap_metab_sig$gene,]$log2FoldChange

rownames(heatmap_metab_sig) <- heatmap_metab_sig$gene
heatmap_metab_sig <- as.matrix(select(heatmap_metab_sig,-gene))
...
check <- filter(complete, gene == "PA14_52820")
...
treated_sig_down <- intersect(T5T0_sig_down$gene,T24T0_sig_down$gene)

treated_sig_down_unique <- setdiff(treated_sig_down, U5U0_sig_down$gene)
treated_sig_down_unique <- setdiff(treated_sig_down_unique, U24U0_sig_down$gene)
...
treated_sig_up <- intersect(T5T0_sig_up$gene,T24T0_sig_up$gene)

treated_sig_up_unique <- setdiff(treated_sig_up, U5U0_sig_up$gene)
treated_sig_up_unique <- setdiff(treated_sig_up_unique, U24U0_sig_up$gene)
...
# filter all DE data with for metab genes
T5T0_succinate <- T5T0[which(T5T0$gene %in% succinateGenes),]
T24T0_succinate <- T24T0[which(T24T0$gene %in% succinateGenes),]
U5U0_succinate <- U5U0[which(U5U0$gene %in% succinateGenes),]
U24U0_succinate <- U24U0[which(U24U0$gene %in% succinateGenes),]
...
succinate_sig_genes<- unique(select(rbind(T5T0_succinate_sig,U5U0_succinate_sig,T24T0_succinate_sig,U24U0_succinate_sig),gene))

heatmap_succinate_sig <- succinate_sig_genes
heatmap_succinate_sig$gene <- sort(heatmap_succinate_sig$gene)
...
# see piping help here: https://stackoverflow.com/questions/13774773/check-whether-value-exist-in-one-data-frame-or-not
heatmap_succinate_sig[heatmap_succinate_sig$gene %in% T5T0_succinate$gene,]$T5T0_succinate = T5T0_succinate[T5T0_succinate$gene %in% heatmap_succinate_sig$gene,]$log2FoldChange
heatmap_succinate_sig[heatmap_succinate_sig$gene %in% U5U0_succinate$gene,]$U5U0_succinate = U5U0_succinate[U5U0_succinate$gene %in% heatmap_succinate_sig$gene,]$log2FoldChange
heatmap_succinate_sig[heatmap_succinate_sig$gene %in% T24T0_succinate$gene,]$T24T0_succinate = T24T0_succinate[T24T0_succinate$gene %in% heatmap_succinate_sig$gene,]$log2FoldChange
heatmap_succinate_sig[heatmap_succinate_sig$gene %in% U24U0_succinate$gene,]$U24U0_succinate = U24U0_succinate[U24U0_succinate$gene %in% heatmap_succinate_sig$gene,]$log2FoldChange

rownames(heatmap_succinate_sig) <- heatmap_succinate_sig$gene
heatmap_succinate_sig <- as.matrix(select(heatmap_succinate_sig,-gene))
